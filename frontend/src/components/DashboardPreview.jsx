...
// 1. Accept the mouse spring values as props
const DashboardPreview = ({ mouseXSpring, mouseYSpring }) => {
  const invoices = [
  ...
  // 2. We use the props instead of creating local motion values
  // Since the props might be undefined if used directly (not in LandingPage), we provide a fallback transform
  
  // Move the entire card in the direction of the mouse
  const cardX = useTransform(mouseXSpring || 0, [-0.5, 0.5], [-20, 20]);
  const cardY = useTransform(mouseYSpring || 0, [-0.5, 0.5], [-20, 20]);
  
  // Move the badge in the direction of the mouse (slightly more for a 3D parallax feel)
  const badgeX = useTransform(mouseXSpring || 0, [-0.5, 0.5], [-35, 35]);
  const badgeY = useTransform(mouseYSpring || 0, [-0.5, 0.5], [-35, 35]);

  // Subtle tilt just to keep it looking interactive and premium
  const rotateX = useTransform(mouseYSpring || 0, [-0.5, 0.5], ["4deg", "-4deg"]);
  const rotateY = useTransform(mouseXSpring || 0, [-0.5, 0.5], ["-4deg", "4deg"]);

  return (
    // 3. Removed the local onMouseMove handlers
    <div 
    ...
    >
      ...
      <motion.div
        ...
        className="w-full max-w-[420px] bg-white/95 dark:bg-slate-900/85 backdrop-blur-2xl rounded-3xl p-4 sm:p-5 shadow-2xl shadow-slate-200 dark:shadow-[0_18px_45px_rgba(15,23,42,0.8)] border border-slate-100 dark:border-slate-800 relative overflow-hidden transition-shadow duration-300 z-20"
      >
        {/* Soft background */}
        ...
        <div className="relative flex items-center justify-between mb-4" style={{ transform: "translateZ(30px)" }}>
          <div>
          ...
          <motion.div
            ...
            className="h-10 w-10 sm:h-11 sm:w-11 bg-indigo-50 dark:bg-slate-800/80 rounded-2xl flex items-center justify-center text-indigo-600 dark:text-indigo-300 shadow-md shadow-indigo-100 dark:shadow-[0_0_20px_rgba(79,70,229,0.55)] cursor-pointer"
          >
        ...
        <div className="relative grid grid-cols-2 gap-2 mb-4 text-[11px]" style={{ transform: "translateZ(20px)" }}>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800" style={{ transform: "translateZ(15px)" }}>
          ...
          <div className="flex items-end gap-1 h-10 bg-slate-50/80 dark:bg-slate-900/80 rounded-2xl px-2 py-1.5 overflow-hidden">
            {[30, 60, 45, 80, 55, 90, 100].map((h, i) => (
              <motion.div
                ...
                className="flex-1 bg-gradient-to-t from-indigo-200 via-indigo-400 to-emerald-400 dark:from-slate-700 dark:via-indigo-500 dark:to-emerald-400 rounded-t-md transition-transform duration-200 origin-bottom"
                style={{ height: `${h}%` }}
              />
            ))}
          </div>
